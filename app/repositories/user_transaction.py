...
class UserTransactionRespository(object):
    ...
    def get_balance(self, user_id):
        user_transactions = m.UserTransaction.objects(userId=user_id)
    ...
    def get_balance_of_address(self, user_id, address):
        deposit_transactions = m.UserTransaction.objects(userId=user_id, toAddress=address, transactionType='deposit')
        total_deposit = sum([tx.amount for tx in deposit_transactions])
        withdrawal_transactions = m.UserTransaction.objects(userId=user_id, fromAddress=address, transactionType='withdrawal')
